...
class BaseTrainer(object):
    ...
    def __init__(self, args):
        ...
        if args.model_name == "alexnet":
            self.passport_config = json.load(
                open(root + "configs/alexnet_passport.json")
            )
    ...
    def get_expid(self, logdir, prefix):
        exps = [
            ...
            if os.path.isdir(os.path.join(logdir, d)) and prefix in d
        ]
    ...
    def makedirs_or_load(self):
        ...
        if not self.eval:
            ...
            self.logdir = os.path.join(self.logdir, str(self.exp_id))
    ...
    def save_model(self, filename, model=None):
        ...
        torch.save(
            model.cpu().state_dict(), os.path.join(self.logdir, f"models/{filename}")
        )
        model.to(self.device)

    def load_model(self, filename):
        self.model.load_state_dict(
            torch.load(os.path.join(self.logdir, f"models/{filename}"))
        )
